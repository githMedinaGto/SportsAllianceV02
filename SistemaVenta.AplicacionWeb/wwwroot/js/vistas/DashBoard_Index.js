    ...
    fetch("/DashBoardController1/ObtenerResumen")
        ...
        .then(responseJson => {
            ...
            if (responseJson.estado) {
                ...
                $("#totalIngresos").text(d.totaIngresos)
                $("#totalProductos").text(d.totalProductos)
                $("#totalCategorias").text(d.toatalCategorias)

                //obtener textos y valores para nuestra grafica de barras
                let barchart_labels;
                let barchart_data;
                ...
                //obtener textos y valores para nuestra grafica de pie
                let piechar_labels;
                let piechar_data;

                if (d.productosTopUltimaSemana.length > 0) {
                    piechar_labels = d.productosTopUltimaSemana.map((item) => { return item.producto })
                    piechar_data = d.ventasUltimaSemana.map((item) => { return item.cantidad })
                } else {
